...
module.exports = async (req, res) => {
    ...
    try {
        if (endpoint) {
            ...
            // CEK 1: Apakah ini halaman Episode (Ada Link Download)?
            if ($('.download').length > 0) {
                ...
                return res.status(200).json({ type: 'episode', endpoint, download_links: downloadLinks });
            } 
            
            // CEK 2: Apakah ini halaman Anime (Ada Daftar Episode)?
            else if ($('.episodelist').length > 0) {
                $('.episodelist ul li').each((i, el) => {
                    const epTitle = $(el).find('a').text().trim();
                    const epEndpoint = $(el).find('a').attr('href')?.split('/').filter(Boolean).pop();
                    if (epTitle) episodeList.push({ title: epTitle, endpoint: epEndpoint });
                });
                return res.status(200).json({ type: 'anime', endpoint, episodes: episodeList });
            }
        }
        ...
        $(selector).each((i, el) => {
            ...
            const ep = $(el).find('a').attr('href')?.split('/').filter(Boolean).pop();
